...
class Client {
  ...
  getPagedResults(path, opts) {
      return new Promise((resolve, reject) => {
        var dataresult = []
        this.getPagedResultsPage(path, opts, 1, function(err,finished,data) {
          dataresult = dataresult.concat(data);
          err && reject(err)
          finished && data && resolve(dataresult)
    });
  });
  }
